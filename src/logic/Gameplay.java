...
public class Gameplay {
    ...
    private Gameplay() {
        ...
        PlanetAction selectPlanet = new PlanetAction(PLANET_SELECT) {
            ...
            @Override
            public boolean doAction(Object sender, long delay, AbstractPlanet planet, Player p) {
        ...
        PlanetAction attackPlanet = new PlanetAction(PLANET_ATTACK) {
            ...
            @Override
            public boolean doAction(Object sender, long delay, AbstractPlanet target, Player p) {
                if (p.hasLost() || currentLevel.isGameOver()) {
                    return false;
                }
                if (p.hasMultiSelectedPlanets()) {
                    return multiAttackPlanet(p.getMultiSelectPlanets(), target, p);

                } else if (p.hasSelectedPlanet()) {
                    return singleAttackPlanet(p.getSelectedPlanet(), target, p);
                } else if (p.hasMultiSelectedShipGroups()) {
                    return multiAttackShipGroup(p.getMultiSelectShipGroups(), target);
                } else if (p.hasSelectedShipGroup()) {
                    return singleAttackShipGroup(p.getSelectedShipGroup(), target);
                }
                return false;
//                target.syncronize(delay);
            }

            private boolean isDistanceToFar(Ranged rangedObject, AbstractPlanet target) {
                float dist = rangedObject.getPosition().
            ...
            private boolean multiAttackPlanet(ArrayList<AbstractPlanet> planets, AbstractPlanet target, Player p) {
            ...
            private boolean multiAttackShipGroup(ArrayList<ShipGroup> ships, AbstractPlanet target) {
            ...
            private boolean singleAttackShipGroup(ShipGroup shipGroup, AbstractPlanet target) {
                if (shipGroup != null) {
            ...
            private boolean singleAttackPlanet(AbstractPlanet selection, AbstractPlanet target, Player p) {
