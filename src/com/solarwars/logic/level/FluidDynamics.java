 ...
 * 
 * Project: solarwars
 * File: FluidDynamics.java
 * Type: com.solarwars.logic.level.FluidDynamics
 * 
...
public class FluidDynamics {
    ...
    /** fluid viscosity */
    private static final float VISC = 0.0001f; 
    /** diffuse amount */
    ...
    public void start(Random randomizer) {
        rgen = randomizer;
    ...
    public void simulate() {
        ...
        for (int i = 0; i < STAR_COUNT; i++) {
            ...
            if (starx[i] >= FLUID_RES) {
                starx[i] -= FLUID_RES;
            }
            if (stary[i] >= FLUID_RES) {
                stary[i] -= FLUID_RES;
            }
            if (starx[i] < 0) {
                starx[i] += FLUID_RES;
